...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // Extract tokens from the request line
            char *requestLine[3] = {NULL, NULL, NULL };
            requestLine[METHOD] = strtok(line_tokens, " ");
            int i = 0;
            ...
            memcpy(
                query, 
                (strncmp(queryInd, "\0", 1) == 0) ? queryInd : queryInd + 1,
                (eosInd - queryInd < 1) ? 1 : eosInd - queryInd);
            
            // Get absolute path
            queryInd = strchrnul(requestLine[REQUEST_TARGET], '?');
            char absolutePath[queryInd- requestLine[REQUEST_TARGET] + 1];
            strncpy(
                absolutePath, 
                requestLine[REQUEST_TARGET], queryInd - requestLine[REQUEST_TARGET]
            );
            absolutePath[queryInd - requestLine[REQUEST_TARGET]] = '\0';
            
            // Absolute path missing a '.'
